...
import { vacacionesMock, DIAS_VACACIONES_POR_ANO } from '../../infrastructure/mock/vacacionesData';
...
const TablaVacMes = ({ year, month, empleados, vacaciones }) => {
...
export const VacacionesTab = () => {
  ...
  // Mostrar 3 meses a la vez (configurables)
  ...
  // Vacaciones del año seleccionado
  const vacacionesAño = useMemo(
    () => vacacionesMock.filter(v => v.año === año),
    [año]
  );

  const vacacionesAñoAnterior = useMemo(
    () => vacacionesMock.filter(v => v.año === año - 1),
    [año]
  );
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-6">
        <div>
          <h2 className="text-xl font-extrabold text-blue-900 tracking-tight uppercase">
            Vacaciones Correspondiente al {año}
          </h2>
          <p className="text-gray-400 text-xs mt-0.5">
            Registro de días de vacaciones tomados por cada colaborador. Cada año corresponden {DIAS_VACACIONES_POR_ANO} días.
          </p>
        </div>
        {/* Selector año */}
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-gray-700 min-w-[60px] text-center">{año}</span>
          <button onClick={() => setAño(a => a + 1)}
            className="w-8 h-8 flex items-center justify-center bg-white border border-gray-200 rounded-lg hover:bg-gray-50 text-gray-600 font-bold shadow-sm transition-all">›</button>
        </div>
      </div>
